...
BEGIN
    IF to_regclass('identity.users') IS NOT NULL
       AND NOT EXISTS (
           SELECT 1
           FROM pg_constraint
           WHERE conname = 'fk_capital_adjustments_created_by'
             AND conrelid = 'resourcecapital.capital_adjustments'::regclass
       ) THEN
        ALTER TABLE resourcecapital.capital_adjustments
            ADD CONSTRAINT fk_capital_adjustments_created_by
...
BEGIN
    IF to_regclass('identity.users') IS NOT NULL
       AND NOT EXISTS (
           SELECT 1
           FROM pg_constraint
           WHERE conname = 'fk_capital_adjustments_updated_by'
             AND conrelid = 'resourcecapital.capital_adjustments'::regclass
       ) THEN
        ALTER TABLE resourcecapital.capital_adjustments
            ADD CONSTRAINT fk_capital_adjustments_updated_by
...
BEGIN
    IF to_regclass('identity.users') IS NOT NULL
       AND NOT EXISTS (
           SELECT 1
           FROM pg_constraint
           WHERE conname = 'fk_capital_allocations_created_by'
             AND conrelid = 'resourcecapital.capital_allocations'::regclass
       ) THEN
        ALTER TABLE resourcecapital.capital_allocations
            ADD CONSTRAINT fk_capital_allocations_created_by
            FOREIGN KEY (created_by)
            REFERENCES identity.users (id)
            ON DELETE RESTRICT;
    END IF;
...
BEGIN
    IF to_regclass('identity.users') IS NOT NULL
       AND NOT EXISTS (
           SELECT 1
           FROM pg_constraint
           WHERE conname = 'fk_capital_allocations_updated_by'
             AND conrelid = 'resourcecapital.capital_allocations'::regclass
       ) THEN
        ALTER TABLE resourcecapital.capital_allocations
            ADD CONSTRAINT fk_capital_allocations_updated_by
            FOREIGN KEY (updated_by)
            REFERENCES identity.users (id)
            ON DELETE RESTRICT;
    END IF;
